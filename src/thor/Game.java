 ...
 * Class used to create all possible actions given a state and turn.
 *
 */
public class Game {
	...
	public List<List<Integer>> produce_actions(BitState state) {
		...
		if (state.getTurn() == Turn.WHITE) {
			...
			int poss_actions_mask = (~state.getWhite_bitboard()[r] & this.possible_actions_hor[r][c]); // Horizontal
																										// actions
			...
			int white_column = Utils.build_column(state.getWhite_bitboard(), curr_pos_mask); // Building column given
																								// position
				...
				if (state.getWhite_bitboard()[r] != 0) {
					...
					for (c = 0; c < state.getWhite_bitboard().length; c++) {
						...
						if ((state.getWhite_bitboard()[r] & curr_pos_mask) == curr_pos_mask) {
							...
							white_column = Utils.build_column(state.getWhite_bitboard(), curr_pos_mask); // Building
																											// column
																											// given
																											// position
				...
				if (state.getBlack_bitboard()[r] != 0) {
					...
					for (c = 0; c < state.getBlack_bitboard().length; c++) {
						...
						if ((state.getBlack_bitboard()[r] & curr_pos_mask) == curr_pos_mask) {
							...
							int white_column = Utils.build_column(state.getWhite_bitboard(), curr_pos_mask); // Building
																												// column
																												// given
																												// position
							int black_column = Utils.build_column(state.getBlack_bitboard(), curr_pos_mask);
